...
- gestió de contingut o dades (**EN CURS**): `llocs` i després `favorits`; la Iteració 6 redefineix el catàleg territorial com a base europea multicultural i multilingüe: subfases 0–V completades i validades amb els pilots Espanya + Alemanya, VI següent i VII–IX pendents. Detall a `docs/ca/funcional-ca.md` (§3.15.7) i `docs/ca/iteracio-6-model-territorial-ca.md`.
...
- Fase IV, Iteració 6 — **REMODELACIÓ TERRITORIAL EUROPEA MULTICULTURAL/MULTILINGÜE — SUBFASE V VALIDADA**. Subfases 0–V **COMPLETADES** amb Espanya i Alemanya com a pilots; subfase VI, **SEGÜENT**; subfases VII–IX, **PENDENTS**. El nucli territorial, les configuracions EF Core/PostgreSQL, les migracions additives i el motor genèric d'importació XLSX estan implementats i provats. Els pilots s'han llegit i canonicalitzat de manera controlada, però no s'han publicat datasets reals, no s'ha fet backfill i el runtime actual de City/GeoNames es conserva.
