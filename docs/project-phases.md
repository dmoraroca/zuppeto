...
- gestió de contingut o dades (**EN CURS**): `llocs` i després `favorits`; la Iteració 6 redefineix el catàleg territorial com a base europea multicultural i multilingüe: subfases 0–V completades, VI en curs i pendent de validació manual final, VII no iniciada i VIII–IX pendents. Detall a `docs/ca/funcional-ca.md` (§3.15.7) i `docs/ca/iteracio-6-model-territorial-ca.md`.
...
- Fase IV, Iteració 6 — **REMODELACIÓ TERRITORIAL EUROPEA MULTICULTURAL/MULTILINGÜE — SUBFASE VI EN CURS / PENDENT VALIDACIÓ MANUAL FINAL**. La subfase VII **NO S’HA INICIAT**; les subfases VIII–IX continuen **PENDENTS**. Fase VI inclou Data Preview, Catàleg territorial refinat, manteniment auditat, overrides limitats, API i un únic autocomplete País → Localitat integrat. No s’han publicat datasets reals, no s’ha fet backfill i City/GeoNames es preserven internament.
