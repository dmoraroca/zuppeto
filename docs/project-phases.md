...
- gestió de contingut o dades (**EN CURS**): `llocs` i després `favorits`; la Iteració 6 redefineix el catàleg territorial com a base europea multicultural i multilingüe i queda **READY FOR IMPLEMENTATION**: subfases 0–III completades amb els pilots Espanya + Alemanya, IV següent i V–IX pendents. Detall a `docs/ca/funcional-ca.md` (§3.15.7) i `docs/ca/iteracio-6-model-territorial-ca.md`.
...
- Fase IV, Iteració 6 — **REMODELACIÓ TERRITORIAL EUROPEA MULTICULTURAL/MULTILINGÜE — READY FOR IMPLEMENTATION**. Subfases 0–III, **COMPLETADES** amb Espanya i Alemanya com a pilots; subfase IV, **SEGÜENT**; subfases V–IX, **PENDENTS**. El contracte funcional queda validat contra els dos pilots i dissenyat per ser extensible, però no empíricament validat contra tota Europa. Els altres països s'auditaran durant la Fase V — Internacionalització. Encara no hi ha model nou, migracions, importacions ni canvis de dades.
