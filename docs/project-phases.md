...
- gestió de contingut o dades (**EN CURS**): `llocs` i després `favorits`; dins la Iteració 6 territorial, VII.9.3 està implementada i validada tècnicament sobre les 8.199 unitats oficials publicades d’Espanya, pendent de validació manual final. Fase VIII i IX no s'han iniciat.
...
- Fase IV, Iteració 6 — **SUBFASE VII EN CURS — VII.9.3 IMPLEMENTADA I VALIDADA TÈCNICAMENT / PENDENT DE VALIDACIÓ MANUAL FINAL**. Espanya és el primer catàleg oficial publicat amb 8.199 unitats i disposa d’explorador lazy; no s’ha republicat durant el refinament. Alemanya continua `Pending` i no publicada. No s’ha fet backfill i City/GeoNames es preserven.
