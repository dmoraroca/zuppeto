...
- gestió de contingut o dades (**EN CURS**): `llocs` i després `favorits`; dins la Iteració 6 territorial, les subfases 0–VI estan completades, VII és la següent i VIII–IX continuen pendents. Detall a `docs/ca/funcional-ca.md` (§3.15.7) i `docs/ca/iteracio-6-model-territorial-ca.md`.
...
- Fase IV, Iteració 6 — **REMODELACIÓ TERRITORIAL EUROPEA MULTICULTURAL/MULTILINGÜE — SUBFASE VI COMPLETADA DEFINITIVAMENT / SUBFASE VII SEGÜENT**. Les subfases VIII–IX continuen **PENDENTS**. Fase VI inclou Data Preview, Catàleg territorial refinat, manteniment auditat, overrides limitats, API i un únic autocomplete País → Localitat integrat i validat. No s’han publicat datasets reals, no s’ha fet backfill i City/GeoNames es preserven internament. La Fase VII encara no s’ha executat.
