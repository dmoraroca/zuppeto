...
- gestió de contingut o dades (**EN CURS**): `llocs` i després `favorits`; la Iteració 6 redefineix el catàleg territorial com a base europea multicultural i multilingüe: subfases 0–VI completades i validades amb els pilots Espanya + Alemanya, VII següent i VIII–IX pendents. Detall a `docs/ca/funcional-ca.md` (§3.15.7) i `docs/ca/iteracio-6-model-territorial-ca.md`.
...
- Fase IV, Iteració 6 — **REMODELACIÓ TERRITORIAL EUROPEA MULTICULTURAL/MULTILINGÜE — SUBFASE VI COMPLETADA DEFINITIVAMENT**. Subfases 0–VI **COMPLETADES**; subfase VII, **SEGÜENT**; subfases VIII–IX, **PENDENTS**. Fase VI inclou Data Preview, Catàleg territorial, manteniment auditat, overrides limitats i contracte País → Localitat. No s’han publicat datasets reals, no s’ha fet backfill i City/GeoNames es preserven transitòriament.
