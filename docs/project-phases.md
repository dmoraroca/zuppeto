...
- gestió de contingut o dades (**EN CURS**): `llocs` i després `favorits`; la Iteració 6 redefineix el catàleg territorial com a base europea multicultural i multilingüe: subfases 0–IV completades i validades amb els pilots Espanya + Alemanya, V següent i VI–IX pendents. Detall a `docs/ca/funcional-ca.md` (§3.15.7) i `docs/ca/iteracio-6-model-territorial-ca.md`.
...
- Fase IV, Iteració 6 — **REMODELACIÓ TERRITORIAL EUROPEA MULTICULTURAL/MULTILINGÜE — SUBFASE IV VALIDADA**. Subfases 0–IV **COMPLETADES** amb Espanya i Alemanya com a pilots; subfase V, **SEGÜENT**; subfases VI–IX, **PENDENTS**. El nucli territorial, les configuracions EF Core/PostgreSQL i la migració additiva estan implementats i provats. No s'han importat datasets, no s'ha fet backfill i el runtime actual de City/GeoNames es conserva.
