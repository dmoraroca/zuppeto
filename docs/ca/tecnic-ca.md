...
**Estat global:** **FASE VI COMPLETADA DEFINITIVAMENT / FASE VII EN CURS — VII.9.3 IMPLEMENTADA I VALIDADA TÈCNICAMENT, PENDENT DE VALIDACIÓ MANUAL FINAL**. Espanya té 8.199 unitats oficials publicades i no s’ha republicat durant VII.9.3. Alemanya continua no publicada, no s’ha fet backfill i GeoNames continua operatiu com a compatibilitat interna.
...
| Espanya | INE, amb denominacions procedents del REL | Operació 30247 — Relación de Municipios y sus Códigos por Provincias | [INEbase](https://www.ine.es/dyngs/INEbase/operacion.htm?c=Estadistica_C&cid=1254736177031&idp=1254734710990&menu=ultiDatos) | XLSX pilot derivat de la relació oficial | Comunitat → província → municipi verificada | 8.132 municipis | No incorporades | CC BY 4.0; gate documental verificat | Permès | INE, data i transformació | `01.01.2026`, publicat `04.02.2026` | **`DatasetSource = Approved` a VII.9.2 / import encara no publicat** |
...
Les migracions `AddTerritorialAsyncWorkerPhase7` i `AddTerritorialPublicationActorPhase7` incorporen cua, artefacte i actor de publicació durable. `AddTerritorialHierarchyReadIndexPhase7` afegeix exclusivament l’índex `ix_territorial_unit_names_unit_id`, justificat per `EXPLAIN ANALYZE`: la cerca real de municipis descendents de Catalunya baixa aproximadament de 647 ms a 22 ms sense modificar cap unitat. El model EF queda alineat amb les migracions. Espanya està publicada; Alemanya continua `Pending` i no publicada.
...
VII.8 queda completada i validada manualment amb backend/PostgreSQL 71/71, Angular 65/65, runner 55/55, builds PASS, ZUP-157 PASS (`sim-20260924T133119211Z-dd6b4f59`), EF Core sense model pendent i `git diff --check` PASS. VII.9.1 documenta la font INE, CC BY 4.0, els usos i l’atribució a [Fonts territorials](iteracio-6-fonts-territorials-ca.md); VII.9.2 registra l’acceptació humana, persisteix `Approved`, preserva Alemanya `Pending` i valida el Pas 6 sense publicar.

### VII.9.3 — explorador del catàleg publicat

El backend afegeix només dos contractes ADMIN de lectura: jerarquia d’una unitat i descendents filtrats per tipus. PostgreSQL resol ancestres i subarbres amb CTE recursives; fills directes, descendents per tipus i recomptes són consultes diferents. La cerca/paginació es fa al servidor i evita N+1. El test PostgreSQL cobreix comunitat, província, municipi terminal, cerca, paginació, recompte i una jerarquia alemanya sintètica de més profunditat; Alemanya real no es publica.

Angular encapsula l’arbre a `TerritorialHierarchyExplorerComponent`, que es renderitza directament en activar la pestanya Jerarquia. No hi ha un estat de presentació paral·lel ni un CTA per convertir el camí en explorador: el mateix tree és el navegador. Carrega només la branca necessària, cacheja pàgines de fills durant l’exploració i fusiona els fills carregats d’un ancestre amb el camí conegut sense duplicar el node actual. Manté una pila de detalls perquè tornar no destrueixi l’estat del component. El DOM usa llistes niades per parent-child i `data-depth` determinista; cada grup dibuixa el seu rail vertical i cada fill la unió horitzontal. La unitat del detall conserva `tree-node--current` com a ressaltat, sense text o badge addicional. Els controls natius de node i chevron són navegables amb teclat i cursor de mà, el toggle exposa `aria-expanded`, les càrregues `aria-busy` i el focus és visible. Els skeletons i errors es renderitzen dins el grup fill a la profunditat que carreguen; `prefers-reduced-motion` elimina les transicions i animacions.

Els terminals projecten nom/tipus/codi com a informació estàtica i un botó `node-detail` separat. Aquest botó reutilitza l’output `openUnit(id)` ja connectat a `TerritorialCatalogComponent.openById`, per tant comparteix `catalogDetail(id)`, modal, pila, errors i retorn de context amb la resta de navegació. `isSelectableLocality` no es projecta visualment al tree, però no s’ha eliminat del model ni del catàleg.

La presentació del heartbeat conserva el valor ISO a `datetime` i mostra data/hora local humana. El modal de confirmació no canvia cap contracte backend: només substitueix text intern per text funcional i agrupa `Cancel·lar`/`Confirmar` amb focus segur. L’E2E opt-in `E2E_TERRITORIAL_CATALOG_REAL=true` recorre Chrome → Angular → API → PostgreSQL sobre Espanya exclusivament en lectura.
