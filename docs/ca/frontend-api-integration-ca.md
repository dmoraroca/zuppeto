...
VII.8 queda completada i validada manualment. El detall de Canvis a publicar manté un únic modal amb cinc pestanyes; Jerarquia consulta ancestres i fills directes del ChangeSet no publicat, amb cerca i paginació servidor, sense perdre filtre, pàgina, scroll ni focus extern. VII.9.2 aprova exclusivament la font INE. Una ordre humana posterior publica Espanya una única vegada amb 8.199 unitats; Alemanya continua no publicada.

VII.9.3 connecta directament la pestanya Jerarquia del Catàleg publicat a `GET /catalog/{id}/hierarchy` i `GET /catalog/{id}/descendants`. `TerritorialHierarchyExplorerComponent` és l’únic navegador jeràrquic: els chevrons carreguen, expandeixen i col·lapsen la mateixa branca, sense CTA o vista intermèdia. El component conserva la diferència entre fills directes i descendents per tipus, cacheja branques de manera lazy i delega cerca, paginació i recomptes al servidor. Els detalls descendents s’apilen sense destruir el component d’origen, de manera que tancar-los restaura expansió, cerca i pàgina. Els estats de càrrega són locals i accessibles; aquesta integració no crida cap endpoint d’escriptura ni repeteix la publicació.
