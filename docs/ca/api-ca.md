...
Aquest és el contracte **actual**. La Iteració 6 ha completat el disseny funcional de la remodelació territorial, però encara no l'ha implementat. La possible substitució d'aquest agregat de fonts per una API territorial sobre catàleg PostgreSQL propi queda **EN REVISIÓ** per a la Iteració 7; no hi ha endpoints nous ni s'ha retirat GeoNames.
- `GET /api/places/{id}`
- `POST /api/places`
- `PUT /api/places/{id}`
