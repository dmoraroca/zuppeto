...
Chrome final autocontingut `sim-20260923T204748547Z-8368eb83`: 7 PASS, 0 FAIL/BLOCKED/SKIP després de retirar les fixtures persistents VI.24. Runner: 55/55. Altres navegadors continuen PENDENT. No s’han publicat datasets oficials. Els diagnòstics ignoren únicament l’avís extern conegut de Google sobre `frame-ancestors` quan és explícitament `report-only`; errors CSP efectius i errors de l’aplicació continuen fallant la prova.
...
La comprovació complementària de VII.8 contra API i PostgreSQL reals confirmava cerca, jerarquia i DTO funcional per Balears, Canarias, Almería, Cataluña, Arenys de Mar, Ceuta, Melilla i una branca alemanya de més de dos nivells. En aquell tall no es va publicar cap pilot. A VII.9.2, Chrome real va obrir el Pas 6 de l’import espanyol, va comprovar `Estat de la font: Aprovada`, 8.199 altes, zero errors/conflictes i `Publicar catàleg` habilitat, i va finalitzar explícitament sense clicar-lo. La publicació única posterior d’Espanya i el refinament sense republicació consten a VII.9.3.

### VII.9.3 — E2E de l’explorador publicat

ZUP-160 admet el mode opt-in `E2E_TERRITORIAL_CATALOG_REAL=true`, separat del circuit sintètic anterior. Amb sincronització Excel diferida, executa només lectura sobre el catàleg oficial d’Espanya: no prepara imports, no publica, no reverteix i no fa manteniment. Intercepta únicament la primera lectura per retardar-la 220 ms i comprovar el skeleton; la resposta continua provenint de l’API real.

La correcció UX definitiva queda coberta pel run Chrome `sim-20260924T215825756Z-9c76be71`, 1/1 PASS. Valida les 8.199 unitats i comprova estructura i interacció: Adra produeix `data-depth` 0–3 i ressaltat visual sense badge textual; Aragón mostra Huesca, Teruel i Zaragoza com a germanes; el chevron de Teruel mostra municipis a depth 3 sense duplicar Teruel; Andalucía i Almería s’expandeixen dins del mateix tree. També valida cursor de mà, col·lapse/reexpansió, absència del CTA retirat, Catalunya, Arenys de Mar, `Tots els municipis` server-side i Canarias → `Palmas, Las` → Arrecife terminal. Les captures A–D es conserven a `docs/ca/evidencies/vii-9-3-tree/`. L’execució ha ajornat la sincronització Excel, no ha publicat ni modificat dades i deixa pendent la validació manual visual. Alemanya continua no publicada.

L’ajust posterior dels terminals passa al run `sim-20260924T221411732Z-a8e3b0e1`, 1/1. ZUP-160 comprova que el tree no renderitza `Seleccionable`, que els 50 municipis carregats de Teruel tenen acció `Veure detall`, i que Ababuj obre mitjançant el flux existent el detall amb codi `44001`; des d’aquest detall, Jerarquia reconstrueix Espanya → Aragón → Teruel → Ababuj. La captura `E-teruel-veure-detall.png` documenta els botons terminals sense alterar el chevron de Teruel. El run és només lectura i manté la sincronització Excel ajornada.
