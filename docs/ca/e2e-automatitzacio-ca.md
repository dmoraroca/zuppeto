...
Chrome final `sim-20260923T194508457Z-a2a4fafd`: 7 PASS, 0 FAIL/BLOCKED/SKIP. Runner: 55/55. Altres navegadors continuen PENDENT. No s’han publicat datasets oficials. Els diagnòstics ignoren únicament l’avís extern conegut de Google sobre `frame-ancestors` quan és explícitament `report-only`; errors CSP efectius i errors de l’aplicació continuen fallant la prova.

ZUP-160 incorpora, sota `E2E_TERRITORIAL_REAL=true`, un recorregut focalitzat Angular → API → Application → EF → PostgreSQL. Les fixtures SQL creen només països i localitats E2E sintètics, comproven catàleg, manteniment, before/after i el nou autocomplete a Perfil, Llocs, Favorits, ADMIN User, ADMIN Place i explorador públic. Verifica un únic `select` de País, un únic combobox de Localitat, absència del botó intern `Cercar`, preservació del `Cercar` general i absència de textos transitoris. VI.24-E afegeix a Llocs i Favorits comprovacions de columna ocupada al 100%, alçades equivalents, amplada relativa superior per Cerca i Localitat, absència de solapaments i overflow, una fila a 1280 px, dues columnes a 900 px i una columna a 600 px. El cicle final passa 1/1 a `sim-20260923T202548369Z-35b2d77c`. Després de la prova es restableixen 2 països, 3 tipus, 3 unitats actives, 3 noms i 2 codis, sense auditoria residual. L’Excel funcional s’ha preservat.

La Fase VI continua oberta i pendent de validació manual final. No s’ha iniciat la Fase VII.
