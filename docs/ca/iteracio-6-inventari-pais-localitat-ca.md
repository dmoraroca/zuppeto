...
| Preview públic del login | `login-page`, `territorial-location-selector`, `PlaceService` | filtre territorial de demos de Places | Sí | Adaptat al selector compartit i al contracte per IDs, amb fallback transitori visible. | Angular + E2E territorial |
...
| E2E i factories | `e2e/scenarios`, adapters/factories | fixtures de User, Place, City i territori | Depèn del flux | Fixtures històriques preservades; territorial usa fixtures sintètiques amb país explícit. | runner 55/55; Chrome territorial 7/7 |
