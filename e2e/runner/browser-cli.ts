...
const workbookPath = join(repositoryRoot, 'docs/probes-e2e/probes-pagines/MAIN_PROBES_ZUPETTO.xlsx');
...
export async function runBrowser(target: BrowserTarget, launcher: BrowserLauncher, args: readonly string[]): Promise<void> {
  ...
  const environment = await loadPilotEnvironment(join(process.cwd(), '.env.e2e.local'));
  const runsDirectory = target.runsDirectoryName === undefined ? join(process.cwd(), '.runs') : join(process.cwd(), '.runs', target.runsDirectoryName);
  ...
  const reporter = new ChromeExcelReporter(new ExcelWorkbookSync(workbookPath), executor, catalog, commit(), target);
...
function parseOptions(args: readonly string[]): { readonly block?: ChromeBlock; readonly scenario?: string; readonly headed: boolean; readonly resume: boolean; readonly runId?: string } {
  const read = (name: string) => args.find((arg) => arg.startsWith(`${name}=`))?.slice(name.length + 1);
  return { block: read('--block') as ChromeBlock | undefined, scenario: read('--scenario'), headed: args.includes('--headed'), resume: args.includes('--resume'), runId: read('--run-id') };
}
