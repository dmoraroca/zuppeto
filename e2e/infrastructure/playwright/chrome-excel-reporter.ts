...
export class ChromeExcelReporter {
  ...
  public constructor(private readonly excel: ExcelWorkbookSync, private readonly executor: ChromePlaywrightExecutor, private readonly catalog: ChromeScenarioCatalog, private readonly commit: string, private readonly browser: BrowserTarget = { key: 'chrome', name: 'Chrome', engine: 'Blink' }) {}
  public async synchronize(records: readonly ExecutionRecord[]): Promise<void> {
    for (const record of records) {
      ...
      await this.excel.synchronize({
        ...
        variant: scenario.inventory.variant, engine: this.browser.engine, browserVersion: this.executor.version(), environment: 'LOCAL', commit: this.commit,
